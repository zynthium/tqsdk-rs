...
| `tqsdk_rs::marketdata::{TqApi, MarketDataState, SymbolId}` | 无 direct replacement；外部统一走 `Client::{quote,kline_ref,tick_ref,wait_update,wait_update_and_drain}` | `marketdata` 已退回 crate 内部实现细节，避免形成第二套 live session 入口 |
