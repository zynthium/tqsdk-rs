...
当前版本的 canonical public contract 已收口到下面几条主路径：
...
- `ReplaySession::runtime(accounts)` 会规范化账户集合（trim、去空、去重、忽略顺序）；首次初始化后，后续调用必须提供同一账户集合，否则返回 `InvalidParameter`。
...
let bars = session.kline("SHFE.au2602", Duration::from_secs(60), 32).await?;
...
| 示例文件 | 主要内容 | 额外环境变量 |
|------|------|------|
| `quote.rs` | Quote、单合约 K 线 serial、多合约对齐 K 线 serial、Tick serial | `TQ_AUTH_USER`、`TQ_AUTH_PASS`，可选 `TQ_LOG_LEVEL`、`TQ_QUOTE_AU`、`TQ_QUOTE_AG`、`TQ_QUOTE_M` |
...
- 对外 public surface 已收口为状态读取、窗口快照和可靠事件流；不要把旧 callback/channel 当成当前主路径。
...
- 如果你需要严格串行处理，优先使用单一消费路径，不要在消费循环里堆叠太多阻塞逻辑。
...
`TqError` 已标记为 `#[non_exhaustive]`，下游如果按 variant 做 `match`，请保留兜底分支。
...
- 提高 `message_queue_capacity()`，或减少消费循环里的阻塞操作。
